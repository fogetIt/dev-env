...
# @Last Modified time: 2018-04-11 11:22:03
: <<'COMMENT'
...
read -p "Rewrite sublime settings? [Y/n]" var && (
    [[ "${var}" == "Y" ]] \
    && cp -rf ${PATH_DEVENV}/editor/sublime/* \
...
ipython  --version || sudo apt-fast -y install ipython ipython-notebook
ipython3 --version || sudo apt-fast -y install ipython3 ipython3-notebook
bpython --version || pip install bpython
...
read -p "To configure vim? [Y/n]" var && (
    ...
    && [[ ! -d ${PATH_SOFTWARES}/plugins ]] \
    && mkdir ${PATH_SOFTWARES}/plugins \
    && git clone git@github.com:VundleVim/Vundle.vim.git ${PATH_SOFTWARES}/plugins/Vundle
    )
